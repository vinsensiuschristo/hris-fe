  ...
  template: `
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">Kelola jenis cuti yang tersedia</p>
      </div>
      <button pButton label="Tambah Jenis Cuti" icon="pi pi-plus" (click)="openDialog()"></button>
    </div>
    ...
    <div class="hris-card">
      @if (loading()) {
        ...
        <p-table [value]="items()" [paginator]="true" [rows]="10" [rowHover]="true">
          <ng-template pTemplate="header"><tr><th>Nama Jenis Cuti</th><th style="width: 120px">Aksi</th></tr></ng-template>
    ...
    <p-dialog [header]="isEditMode ? 'Edit Jenis Cuti' : 'Tambah Jenis Cuti'" [(visible)]="dialogVisible" [modal]="true" [style]="{ width: '400px' }">
      <div class="form-group">
        <label>Nama Jenis Cuti</label>
        <input pInputText [(ngModel)]="formData.namaJenis" placeholder="Contoh: Cuti Tahunan" style="width: 100%" />
      </div>
...
export class LeaveTypeListComponent implements OnInit {
  private service = inject(LeaveTypeService);
  private messageService = inject(MessageService);
  private confirmationService = inject(ConfirmationService);

  items = signal<LeaveType[]>([]);
  ...
  openDialog(): void { this.isEditMode = false; this.editingId = null; this.formData = { namaJenis: '' }; this.dialogVisible = true; }

  editItem(item: LeaveType): void { this.isEditMode = true; this.editingId = item.id; this.formData = { namaJenis: item.namaJenis }; this.dialogVisible = true; }

  saveItem(): void {
    if (!this.formData.namaJenis.trim()) { this.messageService.add({severity:'warn',summary:'Peringatan',detail:'Nama wajib diisi'}); return; }
  ...
  confirmDelete(item: LeaveType): void {
    this.confirmationService.confirm({
      message: `Hapus jenis cuti "${item.namaJenis}"?`, header: 'Konfirmasi Hapus', icon: 'pi pi-exclamation-triangle',
